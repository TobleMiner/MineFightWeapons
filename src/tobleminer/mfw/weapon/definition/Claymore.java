...
import tobleminer.mfw.weapon.explosive.WpClaymore;
	...
	public void onEvent(Match m, Event event)
	{
		if (event instanceof PlayerDropItemEvent)
		{
			...
			if (Main.papi.getProtections().isLocProtected(item.getLocation())
					&& !Main.config.claymore.ignoreProtection(item.getWorld()))
			{
		...
		else if (event instanceof PlayerPickupItemEvent)
		{
			...
			if (clay == null)
				return;
			ppie.setCancelled(true);
			Debugger.writeDebugOut(String.format("%s is trying to pickup a claymore: Owner: %s",
					ppie.getPlayer().getName(), clay.owner.thePlayer.getName()));
			if (player != clay.owner)
			{
				if (player.getTeam() != clay.owner.getTeam() && Main.config.claymore.canEnemyPickup(m.getWorld())
						&& player.thePlayer.isSneaking())
				{
					Debugger.writeDebugOut(
							String.format("%s is picking a hostile claymore.", ppie.getPlayer().getName()));
					this.remove(clay);
					clay.remove();
				...
				{
					Debugger.writeDebugOut(String.format("%s is near a hostile claymore. That's going to hurt.",
							ppie.getPlayer().getName()));
					this.remove(clay);
					clay.explode();
					clay.remove();
				}
