...
public class WpTurret
{
	...
	public Arrow shoot(Location target)
	{
		...
		if (inv.containsAtLeast(new ItemStack(Main.config.turret.getAmmoMaterial(dispenser.getWorld()), 1,
				(short) Main.config.turret.getAmmoSubId(dispenser.getWorld())), 1))
		{
			...
			arr.setVelocity(vel);
			inv.removeItem(new ItemStack(Main.config.turret.getAmmoMaterial(dispenser.getWorld()), 1,
					(short) Main.config.turret.getAmmoSubId(dispenser.getWorld())));
			return arr;
		}
	...
	public TurretMissile shootMissile(Location target)
	{
		...
		Inventory inv = dispenser.getInventory();
		if (inv.containsAtLeast(new ItemStack(Main.config.turret.getMissileMaterial(dispenser.getWorld()), 1,
				(short) Main.config.turret.getMissileSubId(dispenser.getWorld())), 1))
		{
			...
			Vector vel = dir.clone().multiply(speed / dir.length());
			Arrow arr = dispenser.getWorld().spawnArrow(shootLoc, vel, (float) speed, 1.0F);
			TurretMissile missile = new TurretMissile(match, dir, this,
					Main.config.turret.getMissileBlastpower(dispenser.getWorld()), arr, speed);
			inv.removeItem(new ItemStack(Main.config.turret.getMissileMaterial(dispenser.getWorld()), 1,
					(short) Main.config.turret.getMissileSubId(dispenser.getWorld())));
			return missile;
		}
		Debugger.writeDebugOut("Not enough resources available");
		return null;
	...
	{
		return dispenser.getBlock().getType().equals(Main.config.turret.getMaterial(dispenser.getWorld()));
	}
